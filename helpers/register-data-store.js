...
module.exports = require('machine').build({
  ...
  fn: function registerDataStore(inputs, exits) {
    ...
    if (!_.has(inputs.config, 'url')) {
      var url = 'mssql://';
      var port = inputs.config.port || '1433';
      ...
      // For Azure databases
      if(_.has(inputs.config, "options") 
        && _.has(inputs.config.options, "encrypt")
    ...
    Helpers.connection.createManager(inputs.config.url, inputs.config)
      .then(report => {
